...
class KnowledgeBaseRecord(Base):
    ...
    updated_at: Mapped[datetime] = mapped_column(
        "updated_at", DateTime(timezone=False), default=utc_now, onupdate=utc_now, nullable=False
    )
    __table_args__ = ()
...
class KnowledgeSourceRecord(Base):
    ...
    knowledge_base_id: Mapped[str] = mapped_column(
        "knowledge_base_id", Uuid(as_uuid=False), ForeignKey("rag_knowledge_bases.id", ondelete="CASCADE"), nullable=False
    )
    source_type: Mapped[KnowledgeSourceType] = mapped_column(
        Enum(KnowledgeSourceType, name="KnowledgeSourceType"), default=KnowledgeSourceType.UPLOAD, nullable=False
    )
    name: Mapped[str] = mapped_column(String(255), nullable=False)
...
class DocumentRecord(Base):
    ...
    source_id: Mapped[str] = mapped_column(
        "source_id", Uuid(as_uuid=False), ForeignKey("rag_knowledge_sources.id", ondelete="CASCADE"), nullable=False
    )
...
class DocumentVersionRecord(Base):
    ...
    document_id: Mapped[str] = mapped_column(
        "document_id", Uuid(as_uuid=False), ForeignKey("rag_documents.id", ondelete="CASCADE"), nullable=False
    )
...
class IndexProfileRecord(Base):
    ...
    embedding_profile_id: Mapped[str] = mapped_column(
        "embedding_profile_id", Uuid(as_uuid=False), ForeignKey("rag_model_profiles.id", ondelete="RESTRICT"), nullable=False
    )
    sparse_profile_id: Mapped[str | None] = mapped_column(
        "sparse_profile_id", Uuid(as_uuid=False), ForeignKey("rag_model_profiles.id", ondelete="RESTRICT"), nullable=True
    )
...
class IndexGenerationRecord(Base):
    ...
    document_version_id: Mapped[str] = mapped_column(
        "document_version_id", Uuid(as_uuid=False), ForeignKey("rag_document_versions.id", ondelete="CASCADE"), nullable=False
    )
    index_profile_id: Mapped[str] = mapped_column(
        "index_profile_id", Uuid(as_uuid=False), ForeignKey("rag_index_profiles.id", ondelete="RESTRICT"), nullable=False
    )
...
class IngestionJobRecord(Base):
    ...
    generation_id: Mapped[str | None] = mapped_column(
        "generation_id", Uuid(as_uuid=False), ForeignKey("rag_index_generations.id", ondelete="SET NULL"), nullable=True
    )
    ...
    status: Mapped[IngestionJobStatus] = mapped_column(
        Enum(IngestionJobStatus, name="IngestionJobStatus"), default=IngestionJobStatus.PENDING, nullable=False
    )
    created_at: Mapped[datetime] = mapped_column("created_at", DateTime(timezone=False), default=utc_now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(
        "updated_at", DateTime(timezone=False), default=utc_now, onupdate=utc_now, nullable=False
...
class OutboxEventRecord(Base):
    ...
    status: Mapped[OutboxEventStatus] = mapped_column(
        Enum(OutboxEventStatus, name="OutboxEventStatus"), default=OutboxEventStatus.PENDING, nullable=False
    )
    created_at: Mapped[datetime] = mapped_column("created_at", DateTime(timezone=False), default=utc_now, nullable=False)
...
def _to_knowledge_source(row: KnowledgeSourceRecord) -> KnowledgeSource:
    return KnowledgeSource(
        id=row.id, tenant_id=row.tenant_id, knowledge_base_id=row.knowledge_base_id,
        source_type=row.source_type, name=row.name, config_ref=row.config_ref,
...
def _to_document(row: DocumentRecord) -> Document:
    return Document(
        id=row.id, tenant_id=row.tenant_id, knowledge_base_id=row.knowledge_base_id,
        source_id=row.source_id, title=row.title, created_at=row.created_at, updated_at=row.updated_at,
    )


def _to_document_version(row: DocumentVersionRecord) -> DocumentVersion:
    return DocumentVersion(
        id=row.id, tenant_id=row.tenant_id, document_id=row.document_id,
        version_number=row.version_number, lifecycle_state=row.lifecycle_state,
        content_checksum=row.content_checksum, source_revision=row.source_revision,
        pipeline_fingerprint=row.pipeline_fingerprint, object_key_raw=row.object_key_raw,
...
def _to_model_profile(row: ModelProfileRecord) -> ModelProfile:
    return ModelProfile(
        id=row.id, profile_kind=row.profile_kind, provider=row.provider, model=row.model,
        dimensions=row.dimensions, version=row.version, is_active=row.is_active,
    )


def _to_index_profile(row: IndexProfileRecord) -> IndexProfile:
    return IndexProfile(
        id=row.id, embedding_profile_id=row.embedding_profile_id,
        sparse_profile_id=row.sparse_profile_id, collection=row.collection,
        dimensions=row.dimensions, distance_metric=row.distance_metric,
        version=row.version, is_active=row.is_active,
    )


def _to_index_generation(row: IndexGenerationRecord) -> IndexGeneration:
    return IndexGeneration(
        id=row.id, tenant_id=row.tenant_id, document_version_id=row.document_version_id,
        index_profile_id=row.index_profile_id, status=row.status,
        created_at=row.created_at, updated_at=row.updated_at,
    )
...
def _to_ingestion_job(row: IngestionJobRecord) -> IngestionJob:
    return IngestionJob(
        id=row.id, tenant_id=row.tenant_id, document_version_id=row.document_version_id,
        generation_id=row.generation_id, stage=row.stage, attempts=row.attempts,
        idempotency_key=row.idempotency_key, trace_id=row.trace_id, status=row.status,
        created_at=row.created_at, updated_at=row.updated_at,
    )
...
def _to_outbox_event(row: OutboxEventRecord) -> OutboxEvent:
    return OutboxEvent(
        ...
        resource_id=row.resource_id, generation_id=row.generation_id,
...
class SqlAlchemyKnowledgeBaseRepository:
    ...
    async def create(
        ...
        row = KnowledgeBaseRecord(
            id=str(uuid4()), tenant_id=tenant_id, slug=slug, name=name,
    ...
    async def create(
        ...
        *,
        ...
        row = KnowledgeSourceRecord(
            id=str(uuid4()), tenant_id=tenant_id, knowledge_base_id=knowledge_base_id,
            source_type=KnowledgeSourceType(source_type), name=name, config_ref=config_ref,
        )
        self._session.add(row)
        await self._session.commit()
        await self._session.refresh(row)
    ...
    async def create(
        ...
        *,
        ...
        row = DocumentVersionRecord(
            ...
            version_number=version_number, content_checksum=content_checksum,
            object_key_raw=object_key_raw, source_revision=source_revision,
            pipeline_fingerprint=pipeline_fingerprint, size_bytes=size_bytes, mime_type=mime_type,
        )
        self._session.add(row)
        await self._session.commit()
        await self._session.refresh(row)
...
class SqlAlchemyModelProfileRepository:
    ...
    async def create(
        self, *, tenant_id: str, profile_kind: str, provider: str, model: str,
        dimensions: int | None, version: str,
    ) -> ModelProfile:
        row = ModelProfileRecord(
            id=str(uuid4()), tenant_id=tenant_id, profile_kind=profile_kind,
            provider=provider, model=model, dimensions=dimensions, version=version,
        )
        self._session.add(row)
        await self._session.commit()
        await self._session.refresh(row)
...
class SqlAlchemyIndexProfileRepository:
    ...
    async def create(
        self, *, tenant_id: str, embedding_profile_id: str, sparse_profile_id: str | None,
        collection: str, dimensions: int, distance_metric: str, version: str,
    ) -> IndexProfile:
        row = IndexProfileRecord(
            id=str(uuid4()), tenant_id=tenant_id, embedding_profile_id=embedding_profile_id,
            sparse_profile_id=sparse_profile_id, collection=collection, dimensions=dimensions,
            distance_metric=distance_metric, version=version,
        )
        self._session.add(row)
        await self._session.commit()
        await self._session.refresh(row)
...
class SqlAlchemyIndexGenerationRepository:
    ...
    async def create(
        self, *, tenant_id: str, document_version_id: str, index_profile_id: str, status: str
    ) -> IndexGeneration:
        row = IndexGenerationRecord(
            id=str(uuid4()), tenant_id=tenant_id, document_version_id=document_version_id,
            index_profile_id=index_profile_id, status=IndexGenerationStatus(status),
        )
        self._session.add(row)
        await self._session.commit()
        await self._session.refresh(row)
...
class SqlAlchemyIngestionJobRepository:
    ...
    async def create(
        self, *, tenant_id: str, document_version_id: str, generation_id: str | None,
        stage: str, idempotency_key: str, trace_id: str | None,
    ) -> IngestionJob:
        row = IngestionJobRecord(
            id=str(uuid4()), tenant_id=tenant_id, document_version_id=document_version_id,
            generation_id=generation_id, stage=stage, idempotency_key=idempotency_key,
            trace_id=trace_id, status=IngestionJobStatus.PENDING,
        )
        self._session.add(row)
        await self._session.commit()
        await self._session.refresh(row)
...
class SqlAlchemyOutboxEventRepository:
    ...
    async def create(
        self, *, tenant_id: str, event_type: str, resource_id: str | None,
        generation_id: str | None, idempotency_key: str, trace_id: str | None,
        payload: dict[str, object],
    ) -> OutboxEvent:
        row = OutboxEventRecord(
            id=str(uuid4()), tenant_id=tenant_id, event_type=event_type, resource_id=resource_id,
            generation_id=generation_id, idempotency_key=idempotency_key, trace_id=trace_id,
            payload=payload, status=OutboxEventStatus.PENDING,
        )
        self._session.add(row)
        await self._session.commit()
        await self._session.refresh(row)
...
class ChunkRecord(Base):
    ...
    generation_id: Mapped[str] = mapped_column(
        "generation_id", Uuid(as_uuid=False), ForeignKey("rag_index_generations.id", ondelete="CASCADE"), nullable=False
    )
    parent_id: Mapped[str | None] = mapped_column(
        "parent_id", Uuid(as_uuid=False), ForeignKey("rag_chunks.id", ondelete="SET NULL"), nullable=True
    )
    chunk_type: Mapped[ChunkType] = mapped_column(
        Enum(ChunkType, name="ChunkType"), default=ChunkType.CHILD, nullable=False
    )
...
def _to_chunk(row: ChunkRecord) -> Chunk:
    return Chunk(
        id=row.id, tenant_id=row.tenant_id, document_version_id=row.document_version_id,
        generation_id=row.generation_id, parent_id=row.parent_id, chunk_type=row.chunk_type,
        hierarchy_path=tuple(row.hierarchy_path), page=row.page, content_type=row.content_type,
        text=row.text, token_count=row.token_count, source_offsets_start=row.source_offsets_start,
        source_offsets_end=row.source_offsets_end, chunker_version=row.chunker_version,
...
class SqlAlchemyChunkRepository:
    ...
    async def create_many(self, *, tenant_id: str, chunks: list[dict[str, object]]) -> int:
        for chunk_data in chunks:
            row = ChunkRecord(
                id=str(chunk_data.get("id", str(uuid4()))), tenant_id=tenant_id,
...
def _to_stage_checkpoint(row: StageCheckpointRecord) -> StageCheckpoint:
    return StageCheckpoint(
        id=row.id, tenant_id=row.tenant_id, document_version_id=row.document_version_id,
        generation_id=row.generation_id, stage=row.stage, status=row.status,
        attempts=row.attempts, idempotency_key=row.idempotency_key, trace_id=row.trace_id,
        checkpoint_data=row.checkpoint_data, created_at=row.created_at, updated_at=row.updated_at,
    )
...
class SqlAlchemyStageCheckpointRepository:
    ...
    async def upsert(
        ...
        *,
        ...
        existing = await self.find_by_stage(
            tenant_id=tenant_id, document_version_id=document_version_id, stage=stage
        )
        if existing is not None:
            result = await self._session.execute(
                select(StageCheckpointRecord).where(StageCheckpointRecord.id == existing.id)
            )
            ...
            row = StageCheckpointRecord(
                id=str(uuid4()), tenant_id=tenant_id, document_version_id=document_version_id,
                generation_id=generation_id, stage=stage,
                status=StageCheckpointStatus(status), attempts=attempts,
                idempotency_key=idempotency_key, trace_id=trace_id, checkpoint_data=checkpoint_data,
            )
            self._session.add(row)
        await self._session.commit()
        await self._session.refresh(row)
